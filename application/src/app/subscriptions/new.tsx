...
function guessNameFromEmail(email: string): {
  ...
  const cap = (v: string) => v.charAt(0).toUpperCase() + v.slice(1).toLowerCase();
  return {
...
export default function NewSubscriptionPage() {
  ...
  const { subscriptions: mySubscriptions } = useSubscriptions(
    user?.id ?? null,
  );
  ...
  const heldLongPlans = useMemo(() => {
    ...
    for (const sub of mySubscriptions) {
      ...
      if (tariffId !== undefined && tariffId !== null && tariffIds.has(tariffId)) {
  ...
  const residenceDeptLabel = departments.find(
    (d) => d.code === residenceDept,
  );
        ...
        <View style={s.wrapper}>
            ...
            <View style={s.section}>
              ...
              {target === "self" && hasIdentity && (
                <Text style={s.noteText}>
                  ...
                  depuis votre compte (simulation France Connect). Vous
                  pouvez les modifier si besoin.
                </Text>
              )}
          ...
          {step === "address" && (
            <View style={s.section}>
              ...
              <Text style={s.noteText}>
                Ces départements ont été pré-remplis automatiquement
                (simulation d&apos;un appel aux services de l&apos;État).
                Vous pouvez les modifier si besoin.
              </Text>
          ...
          {step === "status" && (
            <View style={s.section}>
              ...
              <Text style={s.noteText}>
                Statut et numéro de sécurité sociale simulés via les
                services de l&apos;État ; le statut est tiré au hasard à
                chaque simulation.
              </Text>
          ...
          {step === "plan" && (
            <View style={s.section}>
              ...
              {!!errors.plan && (
                <Text style={s.fieldError}>{errors.plan}</Text>
              )}
